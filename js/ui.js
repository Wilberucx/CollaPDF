...
import { truncateName, esc, deleteButtonInnerHtml } from './utils.js';
    ...
    return `
      <div class="document-card" id="card_${d.id}" draggable="true" data-id="${d.id}">
        ...
        <div style="padding:4px 8px 8px">
          <button class="doc-edit-delete-btn ${docs.length > 1 ? '' : 'doc-edit-clear-btn'}" onclick="app.removeDocument('${d.id}')">
            ${deleteButtonInnerHtml(docs.length > 1)}
          </button>
        </div>
      </div>
    `}).join('');
